...
def transform_input_data(tensor_dict,
    ...
    if retain_original_image:
        tensor_dict[fields.InputDataFields.original_image] = tf.cast(
            image_resizer_fn(tensor_dict[fields.InputDataFields.image], None)[
                0],
            tf.uint8)
    ...
    if fields.InputDataFields.pseudo_mask in tensor_dict:
        ...
        resized_pseudo_mask = image_resizer_fn(pseudo_mask)[0]
        tensor_dict[fields.InputDataFields.pseudo_mask] = resized_pseudo_mask
...
def pad_input_data_to_static_shapes(tensor_dict, max_num_boxes, num_classes,
    ...
    padding_shapes = {
        ...
        fields.InputDataFields.groundtruth_classes: [max_num_boxes,
                                                     num_classes],
        fields.InputDataFields.groundtruth_confidences: [
    ...
    if fields.InputDataFields.pseudo_mask in padded_tensor_dict:
        padded_tensor_dict[
            fields.InputDataFields.pseudo_mask] = tf.clip_by_value(
            tf.floor(
                padded_tensor_dict[fields.InputDataFields.pseudo_mask]
            ),
            clip_value_min=0.0,
            clip_value_max=num_classes
        )
...
def augment_input_data(tensor_dict, data_augmentation_options):
    ...
    include_pseudo_mask = (fields.InputDataFields.pseudo_mask in tensor_dict)

    if include_pseudo_mask:
        tensor_dict[fields.InputDataFields.pseudo_mask] = tf.expand_dims(
            tf.to_float(tensor_dict[fields.InputDataFields.pseudo_mask]), 0)

    include_instance_masks = (fields.InputDataFields.groundtruth_instance_masks
                              in tensor_dict)

    include_pesudo_mask = (fields.InputDataFields.pseudo_mask in tensor_dict)
    ...
    tensor_dict = preprocessor.preprocess(
        ...
        func_arg_map=preprocessor.get_default_func_arg_map(
            ...
            include_pseudo_mask=include_pesudo_mask))
    tensor_dict[fields.InputDataFields.image] = tf.squeeze(
        tensor_dict[fields.InputDataFields.image], axis=0)
    if fields.InputDataFields.pseudo_mask in tensor_dict:
        tensor_dict[fields.InputDataFields.pseudo_mask] = tf.squeeze(
            tensor_dict[fields.InputDataFields.pseudo_mask], axis=0)
...
def create_train_input_fn(train_config, train_input_config,
    ...
    def _train_input_fn(params=None):
        ...
        def transform_and_pad_input_data_fn(tensor_dict):
            ...
            return (
            _get_features_dict(tensor_dict), _get_labels_dict(tensor_dict))

        dataset = INPUT_BUILDER_UTIL_MAP['dataset_build'](
            train_input_config,
            transform_input_data_fn=transform_and_pad_input_data_fn,
            batch_size=params[
                'batch_size'] if params else train_config.batch_size)
        return dataset
...
def create_eval_input_fn(eval_config, eval_input_config, model_config):
    ...
    def _eval_input_fn(params=None):
        ...
        dataset = INPUT_BUILDER_UTIL_MAP['dataset_build'](
            eval_input_config,
            batch_size=params[
                'batch_size'] if params else eval_config.batch_size,
            transform_input_data_fn=transform_and_pad_input_data_fn)
        return dataset
